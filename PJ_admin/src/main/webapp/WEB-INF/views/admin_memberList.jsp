			...
			<main>
				...
				<div class="admin_memberListTableDiv">
					<table class="admin_memberListTable">
						...
						<c:forEach items="${map.list}" var="avo">
							<tr onClick="location.href='admin_memberModify?admin_id=${avo.admin_id}'" style="cursor:pointer;">
								<td>${avo.id}</td>
								<td>${avo.admin_id}</td>
								...
								<c:if test="${avo.admin_issuper==0}">
									<td style="font-weight:bold;">일반</td>
								</c:if>
								<c:if test="${avo.admin_ban==1}">
									<td style="color:blue; font-weight:bold;">정상</td>
								</c:if>
								<c:if test="${avo.admin_ban==0}">
